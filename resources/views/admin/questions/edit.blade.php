...
@section('title', 'Edit Soal - Admin SNBTKU')
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">Perbarui soal dalam bank soal</p>
        </div>
                ...
                <div class="space-y-6">
                            ...
                            <select name="subject_id" 
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{ $subject->id }}" 
                                            {{ old('subject_id', $question->topic->subject_id) == $subject->id ? 'selected' : '' }}>
                                        {{ $subject->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @foreach(['a', 'b', 'c', 'd', 'e'] as $option)
                                <div>
                                    ...
                                    <textarea name="option_{{ $option }}" 
                                              ...
                                              required>{{ old('option_'.$option, $question->{'option_'.$option}) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="explanation" 
                                  ...
                                  placeholder="Masukkan pembahasan soal (opsional)">{{ old('explanation', $question->explanation) }}</textarea>
                        @error('explanation')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
...
<script>
    ...
    document.getElementById('subject_id').addEventListener('change', function() {
        ...
        // Reset topic selection if subject changed
        if (selectedSubjectId !== '{{ $question->topic->subject_id }}') {
            topicSelect.value = '';
        }
